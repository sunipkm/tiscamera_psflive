...
namespace imgui_addons
{
    ...
    bool ImGuiFileBrowser::renderNavAndSearchBarRegion()
    {
        ...
        for(long unsigned int i = 0; i < current_dirlist.size(); i++)
        {
            ...
            if(i != current_dirlist.size() - 1)
            {
                ...
                if(ImGui::GetCursorPosX() + next_label_width >= (nw_size.x - style.WindowPadding.x * 3.0))
                {
                    ...
                    if(ImGui::BeginPopup("##NavBarDropboxPopup"))
                    {
                        ...
                        {
                            ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.882f, 0.745f, 0.078f,1.0f));
                            for(long unsigned int j = i+1; j < current_dirlist.size(); j++)
                            {
    ...
    bool ImGuiFileBrowser::renderFileListRegion()
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.882f, 0.745f, 0.078f,1.0f));
        int items = 0;
        for (long unsigned int i = 0; i < filtered_dirs.size(); i++)
        {
        ...
        //Output files
        for (long unsigned int i = 0; i < filtered_files.size(); i++)
        {
    ...
    bool ImGuiFileBrowser::renderInputTextAndExtRegion()
    {
        ...
        if(ImGui::IsItemEdited() || ImGui::IsItemActivated())
        {
            ...
            {
                inputcb_filter_files.clear();
                for(long unsigned int i = 0; i < subfiles.size(); i++)
                {
            ...
            {
                inputcb_filter_files.clear();
                for(long unsigned int i = 0; i < subdirs.size(); i++)
                {
    ...
    void ImGuiFileBrowser::renderExtBox()
    {
        ...
        if(ImGui::BeginCombo("##FileTypes", selected_label ))
        {
            ...
            for(long unsigned int i = 0; i < valid_exts.size(); i++)
            {
                if(ImGui::Selectable(valid_exts[i].c_str(), selected_ext_idx == i && !show_all_files && !show_files_with_valid_extensions))
                {
                    ...
                    if(dialog_mode == DialogMode::SAVE)
                    {
                        ...
                        if(idx == std::string::npos)
                            idx = strlen(input_fn);
                        for(long unsigned int j = 0; j < valid_exts[selected_ext_idx].size(); j++)
